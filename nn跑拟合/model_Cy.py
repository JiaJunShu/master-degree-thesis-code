...
# 读取数据
data = pd.read_csv('my_dataout(1).csv')

# 分离特征和标签
X = data.iloc[:, :4].values  # 选择前四列作为输入特征
y = data.iloc[:, 4:7].values  # 选择第五至第七列作为输出标签（形状为 (n_samples, 3)）
...
joblib.dump(scaler, 'scaler_Cy.pkl')  # 保存 scaler
...
torch.save(model.state_dict(), 'model_Cy.pth')

# 创建模型实例
model = model_Cy()

# 加载模型的状态字典
model.load_state_dict(torch.load('model_Cy.pth'))

# 切换到评估模式
model.eval()

# 假设你要测试的数据输入（请根据实际情况替换数值）
new_data = [[0.5, 120000000, 0.0004361480517093982*180/3.14159, 0.10505534903565272*180/3.14159]]  # 这里是一个示例数据点，使用二维列表
